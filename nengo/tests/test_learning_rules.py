...
def test_generic_rule(Simulator, nl_nodirect, seed, modifies):
    if nl_nodirect == nengo.RectifiedLinear and modifies == "weights":
        # this is a sloppy way of avoiding this one case without having
        # to make a big custom parameterization
        # TODO: why doesn't it work for this case?
        return
    ...
    with m:
        ...
        conn = nengo.Connection(
            ...
            transform=1 if modifies == "decoders" else np.eye(n),
            learning_rule_type=rule)
        nengo.Connection(b, conn.learning_rule[:2], transform=1)
        nengo.Connection(u, conn.learning_rule[:2], transform=1)
